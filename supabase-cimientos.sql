...
drop policy if exists "propio_select" on public.estado;
drop policy if exists "propio_insert" on public.estado;
drop policy if exists "propio_update" on public.estado;
drop policy if exists "propio_delete" on public.estado;

create policy "propio_select" on public.estado
  for select using (auth.uid() = user_id);

create policy "propio_insert" on public.estado
  for insert with check (auth.uid() = user_id);

create policy "propio_update" on public.estado
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);

create policy "propio_delete" on public.estado
  for delete using (auth.uid() = user_id);

-- Marca de tiempo siempre fresca del lado del servidor.
create or replace function public.tocar_estado()
...
drop trigger if exists trg_tocar_estado on public.estado;
create trigger trg_tocar_estado before insert or update on public.estado
  for each row execute function public.tocar_estado();
